...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def create_transactions(transactions_list):
        transaction_objects = []
        for transaction in transactions_list["transaction_list"]:
            transaction_objects.append(Transaction.objects.create(
                user=transactions_list.get("user"),
                amount=transaction.get("amount"),
                description=transaction.get("description"),
                document = transactions_list.get("document"),
            ))
...
class AlertViewSet(viewsets.ModelViewSet):
    ...
    @staticmethod
    def create_alert(alerts_list):
        alert_objects = []
        for alert in alerts_list["transaction_list"]:
            alert_objects.append(Alert.objects.create(
                transaction=alert,
                user=alerts_list.get("user"),
            ))
...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        operation_description="List all documents for the current authenticated user",
        responses={200: DocumentSerializer(many=True)},
    )
    def list(self, request, *args, **kwargs):
        documents = self.get_queryset().filter(userID=request.user)
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'title': openapi.Schema(type=openapi.TYPE_INTEGER, description='Title'),
    ...
    def create(self, request, *args, **kwargs):
        ...
        produce_message("transaction-create",{"transactions":{
